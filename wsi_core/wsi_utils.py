...
Image.MAX_IMAGE_PIXELS = 5005056
import math
import cv2
...
def StitchCoords(hdf5_file_path, wsi_object, downscale=16, draw_grid=False, bg_color=(0,0,0), alpha=-1):
    wsi = wsi_object.getOpenSlide()#wsi图像
    vis_level = wsi.get_best_level_for_downsample(downscale)#指定下采样倍数最好的level
    file = h5py.File(hdf5_file_path, 'r')
    dset = file['coords']
    coords = dset[:]#得到的图像块数
    w, h = wsi.level_dimensions[0]#原始的w，h

    print('start stitching {}'.format(dset.attrs['name']))
    print('original size: {} x {}'.format(w, h))

    w, h = wsi.level_dimensions[vis_level]#下采样下的w，h

    print('downscaled size for stiching: {} x {}'.format(w, h))
    print('number of patches: {}'.format(len(coords)))
    
    patch_size = dset.attrs['patch_size']#patch_size大小
    patch_level = dset.attrs['patch_level']#下采样倍数
    ...
    if alpha < 0 or alpha == -1:
        heatmap = Image.new(size=(w,h), mode="RGB", color=bg_color)#生成热图
    else:
        heatmap = Image.new(size=(w,h), mode="RGBA", color=bg_color + (int(255 * alpha),))
